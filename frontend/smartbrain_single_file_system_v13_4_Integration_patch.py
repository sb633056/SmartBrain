...
# ================================
# Monkey-patch backend KPI engine
# ================================
...
def _smartbrain_backend_call(df, uploaded_file, BACKEND_URL):
    """Send file to backend FastAPI for KPI processing."""
...
def build_kpi_payload(df, uploaded_file=None, BACKEND_URL=None):
    """Wrapper: Call backend if file & URL are provided, else local version."""
    if uploaded_file and BACKEND_URL:
        return _smartbrain_backend_call(df, uploaded_file, BACKEND_URL)
...
def run_smartbrain_ui():

    BACKEND_URL = "https://smartbrain-backend-em92.onrender.com"
    st.set_page_config(page_title='SmartBrain Founder V8', layout='wide')
    st.title("💄 SmartBrain for Beauty Founders — AI-Powered Insights")

    st.markdown(
        "<p style='color:#666; font-size:14px;'>AI-driven beauty & personal care intelligence for growth-focused founders.</p>",
    ...
    with st.sidebar:
        ...
        st.markdown(
            """
    <div style="padding: 8px 0; font-size: 24px; font-weight: bold;">
        ℹ️ About SmartBrain
    </div>
            """, 
            unsafe_allow_html=True
        )
        # Clean, padded description box
        st.markdown(
            """
    <div style="
        ...
        line-height: 1.6;
    ">
    <b>SmartBrain</b> is an AI-powered insights engine built for Beauty & Personal Care founders.
    <br><br>

    <b>Upload your:</b>
    <ul style="margin-top: 6px; margin-bottom: 12px;">
        <li>Sales Data</li>
        <li>Marketing Spend Data</li>
        <li>Inventory Data</li>
        <li>SKU Master</li>
    </ul>
    <b>Get instant intelligence:</b>
    <ul style="margin-top: 6px;">
        <li>Founder-grade KPIs</li>
        <li>AI Summary</li>
        <li>Spend guidance</li>
        <li>Expiry & overstock alerts</li>
    </ul>
    </div>
            """, 
            unsafe_allow_html=True
        ...
        compact = st.checkbox("Compact View (smaller charts)", value=True)


    uploaded = st.file_uploader('Upload combined CSV/Excel (sales+marketing+inventory+master)', type=['csv','xlsx'])
    
    if uploaded is None: st.info('Upload a file or use example to test.')
    ...
    if df is not None:
        ...
        with tab1:
            ...
            def make_json_safe(obj):
                """Recursively convert DataFrames, Series, numpy numbers to JSON-safe types."""
                if isinstance(obj, pd.DataFrame):
                    return obj.to_dict(orient="records")
                if isinstance(obj, pd.Series):
                    return obj.to_dict()
            ...
            if st.button("⚡ Generate AI Commentary"):
                ...
                # 🔥 Convert to JSON-safe format before sending to backend
